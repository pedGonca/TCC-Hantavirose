...
Os dados analisados neste trabalho são provenientes do Sistema de Informação de Agravos de Notificação (SINAN), onde são registradas as informações contidas nas fichas de investigação (anexo)
...
Uma das principais funções probabilísticas usadas para descrever estudo de sobrevivência é a função de sobrevivência, que em termos probabilísticos é escrita como:
...
A principal característica da análise de sobrevivência é a utilização de todas as informações disponíveis, ou seja, tanto os indivíduos de um experimento 
...
Sendo $\Theta = (b,\beta,S_0(t))$ os parâmetros desconhecidos. Deixando $Y$ como o indicador de que um indivíduo vai eventualmente ($Y=1$) ou nunca ($Y=0$) experimentar o evento, com probabilidade de $\pi(z)$. Dado $y = (y_1,y_2,\cdots,y_n)$ e $\textbf{O}$, a função de Verossimilhança pode ser expressa como:
...
Para um entendimento mais detalhado do método recomendo a leitura do artigo escrito por Chao Cai, Yubo Zou, Yingwei Peng and Jiajia Zhanga em 2012 que se encontra nas referência do trabalho. 
